...
import { sanitizeTextInput, sanitizeHTML } from '../../utils/security';

export const AIAnnouncementStudio: React.FC = () => {
  const { createAnnouncementFromPrompt, playSfx, analytics } = useEvent();
  ...
  const preview: AnnouncementChannelVariants = useMemo(() => {
    const clean = sanitizeTextInput(prompt, 300) || 'Important event update for all attendees';
    return {
      push: {
        title: priority === 'urgent' || priority === 'emergency' ? '🚨 EVENTOS URGENT ALERT' : '📢 EventOS Announcement',
      ...
      email: {
        subject: `${priority === 'urgent' || priority === 'emergency' ? '[URGENT] ' : ''}HackMatrix 2026: ${clean.slice(0, 45)}...`,
...
<body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #090D16; color: #ffffff; padding: 24px;">
  <div style="max-width: 580px; margin: 0 auto; background: #0f172a; border: 1px solid #334155; border-radius: 12px; padding: 24px;">
    ...
    <p style="font-size: 16px; line-height: 1.5; color: #cbd5e1;">${sanitizeHTML(clean)}</p>
    <div style="margin-top: 20px; padding: 12px; background: #1e293b; border-radius: 8px; font-size: 13px; color: #94a3b8;">
      ...
      slackDiscord: `@everyone ${priority === 'urgent' || priority === 'emergency' ? '🚨 **URGENT BROADCAST**' : '📢 **ANNOUNCEMENT**'}\n> ${clean}\n\n*Broadcasted via EventOS Mission Control* • *${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}*`
    };
  }, [prompt, priority, analytics.totalCheckedIn]);
  ...
  }, [prompt, priority, playSfx, createAnnouncementFromPrompt]);

  const copyToClipboard = useCallback((text: string, channel: string) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          <form onSubmit={handleBroadcast} className="glass-card rounded-2xl border border-white/10 p-5 space-y-4">
            <div>
              <label htmlFor="raw-announcement-input" className="text-xs font-mono text-slate-300 font-semibold mb-1.5 flex items-center justify-between">
                <span>Raw Organizer Intent:</span>
                <span className="text-[10px] text-cyan-400 font-normal">AI will auto-adapt copy</span>
              </label>
              <textarea
                id="raw-announcement-input"
                rows={4}
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder="e.g. Keynote venue shifted to Hall B immediately..."
                className="w-full bg-slate-950 border border-white/10 rounded-xl p-3.5 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-cyan-400 font-sans resize-none leading-relaxed"
              />
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-mono text-slate-400 block">Broadcast Priority & Auditory Alert:</label>
              <div className="grid grid-cols-4 gap-1.5 font-mono text-xs" role="radiogroup" aria-label="Broadcast Priority">
                {(['normal', 'high', 'urgent', 'emergency'] as const).map((p) => {
                  ...
                  return (
                    <button
                      ...
                      className={`py-2 rounded-xl capitalize font-bold transition-all text-[11px] ${
                        isSelected
                          ? p === 'emergency' || p === 'urgent'
                            ? 'bg-rose-500/20 text-rose-300 border border-rose-500/50 shadow-neon-indigo'
            ...
            >
              {isGenerating ? (
                ...
                <>
                  ...
                  <span>Broadcast Across All 4 Channels Now</span>
                </>
              )}
            </button>
          </form>
        ...
        {/* Right Col (7 cols): Dynamic Multi-Channel Output Mockups */}
        <div className="lg:col-span-7 space-y-4">
          <div className="glass-card rounded-2xl border border-white/10 p-5 space-y-4 min-h-[380px] flex flex-col justify-between">
            
            {/* Channel Tabs */}
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              <span className="text-xs font-mono text-slate-400 uppercase tracking-wider">
                Live Channel Adaptation Previews
              </span>

              <div className="flex items-center gap-1 bg-slate-950 p-1 rounded-xl border border-white/5" role="tablist">
                {[
                  { id: 'push', label: 'Push', icon: <Smartphone className="w-3.5 h-3.5" /> },
                  { id: 'sms', label: 'SMS', icon: <MessageSquare className="w-3.5 h-3.5" /> },
                  { id: 'email', label: 'Email', icon: <Mail className="w-3.5 h-3.5" /> },
                  { id: 'slackDiscord', label: 'Discord', icon: <Radio className="w-3.5 h-3.5" /> },
                ].map((tab) => {
            ...
            <div className="my-auto py-2">
              ...
              {activeChannelTab === 'slackDiscord' && (
                <div className="space-y-3">
                  ...
                  <div className="bg-slate-950 p-4 rounded-xl border border-white/10 font-mono text-xs text-indigo-300 whitespace-pre-wrap leading-relaxed">
            ...
            <div className="pt-3 border-t border-white/5 text-[10px] font-mono text-slate-400 flex items-center justify-between">
              <span>Delivery Status: Ready</span>
