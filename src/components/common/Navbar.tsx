...
export const Navbar: React.FC = () => {
  ...
  const navItems: { id: ViewMode; label: string; icon: React.ReactNode; badge?: string }[] = [
    ...
    { id: 'docs', label: 'Architecture', icon: <FileCode2 className="w-4 h-4 text-emerald-400" /> },
  ];
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-white/10 bg-[#090D16]/85 backdrop-blur-xl">
      <div className="max-w-full mx-auto px-3 sm:px-4 lg:px-6 h-14 flex items-center gap-3 overflow-x-auto pb-1">
        ...
        >
          <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-indigo-600 via-indigo-500 to-cyan-400 p-[1px] shadow-neon-cyan transition-transform duration-300 group-hover:scale-105">
            <div className="w-full h-full bg-[#090D16] rounded-[8px] flex items-center justify-center">
              <Terminal className="w-4 h-4 text-cyan-400 transition-colors group-hover:text-indigo-300" />
            </div>
          </div>
          <div className="hidden sm:block">
            <div className="flex items-center gap-1">
              ...
              <span className="px-1 py-0.5 rounded text-[9px] font-bold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 uppercase tracking-widest font-mono">
                AI v2.4
              </span>
            </div>
        ...
        {/* Nav Links - Always visible, left-aligned, horizontally scrollable on small screens */}
        <nav className="flex items-center gap-1 bg-slate-900/60 border border-white/5 p-1 rounded-xl flex-1 min-w-0 overflow-x-auto scrollbar-hide">
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`relative flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 whitespace-nowrap shrink-0 ${
                  isActive 
        ...
        {/* Right Action Controls - Compact */}
        <div className="flex items-center gap-1.5 shrink-0">
          
          {/* Quick Role Switcher Pill - Compact */}
          <div className="hidden md:flex items-center bg-slate-900/90 border border-white/10 p-0.5 rounded-lg text-[10px] font-medium font-mono">
            <span className="px-1.5 text-slate-400 text-[9px] uppercase tracking-wider">Persona:</span>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-indigo-500/10 hover:bg-indigo-500/25 text-indigo-300 border border-indigo-500/30 text-xs font-mono transition-all hidden sm:flex"
          >
          ...
          >
            <span className={`w-1.5 h-1.5 rounded-full ${isLiveSimulating ? 'bg-emerald-400 animate-pulse' : 'bg-slate-500'}`} />
            <span className="hidden md:inline">{isLiveSimulating ? 'LIVE' : 'PAUSED'}</span>
          </button>
          ...
          <button
            ...
            className="md:hidden p-2 rounded-lg bg-slate-800 border border-white/10 text-slate-300 hover:text-white"
          >
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden border-b border-white/10 bg-slate-950/95 p-4 space-y-3 backdrop-blur-2xl">
